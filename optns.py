...
class OptNS:
    ...
    """
    def __init__(
        self, linear_param_names, nonlinear_param_names, compute_model_components,
        nonlinear_param_transform, linear_param_logprior,
        flat_data, flat_invvar=None, positive=True,
    ):
        """Initialise.
    ...
    def optlinearsample(self, nonlinear_params, size):
        ...
        if self.statmodel.flat_invvar is None:
            linear_params, loglike_proposal, loglike_target = self.statmodel.sample_poisson(X, size)
        ...
        if self.statmodel.flat_invvar is None:
            ...
            logl_full = -0.5 * np.sum(((y_pred - self.statmodel.flat_data) * self.statmodel.flat_invvar)**2, axis=1)
    ...
    def ReactiveNestedSampler(self, **sampler_kwargs):
        self.sampler = ReactiveNestedSampler(
            ...
            self.loglikelihood, transform=self.nonlinear_param_transform,
            **sampler_kwargs)
        return self.sampler
    ...
    def get_weighted_samples(self, oversample_factor):
        ...
        fullsamples = np.empty((Nmaxsamples, len(self.linear_param_names) + len(self.nonlinear_param_names)))
    ...
    def resample(self, fullsamples, weights, y_preds, rng=np.random):
        rejection_sampled_indices = rng.choice(len(weights), p=weights, size=int(ess(weights)))
        return fullsamples[rejection_sampled_indices,:], y_preds[rejection_sampled_indices,:]
